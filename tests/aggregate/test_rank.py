...
def test_rankings_are_parallel_not_composite() -> None:
    ...
    weaknesses = [
        ...
        _weakness(2, "modeling", "state_tracking", frequency=5, trainability=0.9),
        _weakness(3, "surface", "syntax", frequency=1, trainability=0.3),
        _weakness(4, "modeling", "abstraction", frequency=1, trainability=0.3),
    ]

    rankings = build_rankings(weaknesses)

    assert rankings.by_frequency == [1, 2, 3, 4]
    assert rankings.by_difficulty == [2, 4, 1, 3]
    assert rankings.by_trainability == [2, 1, 3, 4]
